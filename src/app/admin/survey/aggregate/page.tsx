...
const TABS = [
  { id: "dates", label: "顔合わせ日別" },
  { id: "respondents", label: "回答者別" },
  { id: "weekday", label: "曜日・時間帯" },
  { id: "period", label: "忙しい時期" },
  { id: "seminar", label: "セミナー希望" },
  { id: "comment", label: "講義・コメント" },
] as const;

export default function SurveyAggregatePage() {
  ...
  const [tab, setTab] = useState<(typeof TABS)[number]["id"]>("dates");
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              tab === t.id ? "bg-amber-500 text-white" : "bg-stone-200 text-stone-600 hover:bg-stone-300"
            }`}
          >
            {t.label}
          </button>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-4">
        {tab === "dates" && (
          <>
            ...
            {candidates.length === 0 ? (
              <p className="text-sm text-stone-400">候補日が未設定です。「設定」で追加してください。</p>
            ) : (
        ...
        {tab === "weekday" && (
          <>
            ...
            <h3 className="font-semibold text-stone-700 mb-3 mt-4">参加しにくい時間帯（人数）</h3>
